...
void Game::LoadDictionary(int word_size) {
...
void Game::PrintGrid(std::ostream& out) {
...
bool Game::CheckGuess(const std::string& game_word) {
  if(!game_grid_) return false;

  return game_grid_->CheckGuess(game_word);
}
